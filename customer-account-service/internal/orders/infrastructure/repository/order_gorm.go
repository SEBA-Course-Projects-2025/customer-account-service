...
func (gor *GormOrderRepository) FindAll(ctx context.Context, params dtos.OrderQueryParams, customerId uuid.UUID) ([]models.Order, error) {

	var orders []models.Order

	db := gor.db.WithContext(ctx).Where("customer_id = ?", customerId).Preload("OrderItems")
...
func (gor *GormOrderRepository) FindById(ctx context.Context, id uuid.UUID, customerId uuid.UUID) (*models.Order, error) {

	var order models.Order

	if err := gor.db.WithContext(ctx).Preload("OrderItems").First(&order, "id = ? AND customer_id = ?", id, customerId).Error; err != nil {
